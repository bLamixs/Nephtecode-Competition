...
make zip
```
Команда собирает чистый и компактный архив **`neftecode_solution.zip`** (весом **< 10 МБ** вместо 3.2 ГБ), автоматически исключая тяжелые сырые CSV (`data/raw/*.csv`), сгенерированные файлы Parquet (`data/processed/*`), журналы запусков (`logs/*`), кэши и виртуальное окружение (`venv/`). В архиве сохраняются весь исходный код, тесты, конфигурации, сценарии, скрипты и документация.
...
| `make zip` | Сборка чистого zip-архива (`neftecode_solution.zip`, <10 МБ) без тяжелых данных, логов и кэшей для отправки жюри |
